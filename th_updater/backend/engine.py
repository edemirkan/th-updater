...
class AppEngine():
    ...
    def version_info(self):
        ...
        self.t.lbl_local_version['text'] = f"Local version: {
            self.config.local_version}"
        self.t.lbl_remote_version['text'] = f"Latest version: {
    ...
    def do_version_update(self, event: Event):
        ...
        if filesys.verify_md5(staging_path, self.artifact):
            ...
            except ZipDecompressError as e:
                self.t.append_to_log("Error!", True)
                self.t.append_to_log(
                    "Unable to extract downloaded file...", True)
                self.t.append_to_log(f"{e}", True)
            else:
                self.t.append_to_log("Done.", True)
